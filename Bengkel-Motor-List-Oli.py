...
def cetak_resi():
    ...
    resi.write("-> Jumlah/Item      : " + str(sum(list_qty)) + " Liter\n")
    resi.write("---------------------------------------\n\n")
    resi.write("-> Daftar Barang\n")
    ...
    while(i < user):
        time.sleep(0.7)
        resi.write("\n")   
        resi.write("    ======== LIST BARANG ( " + str(urutan) + " ) ========\n")
        resi.write("    Nama Barang     : " + str(list_oli[i]) + "\n")
        resi.write("    Harga           : " + uang(list_harga[i]) + " Per Liter\n")
        resi.write("    Kuantitas       : " + str(list_qty[i]) + " Liter\n")
        total = list_harga[i] * list_qty[i]
        resi.write("    Total           : " + uang(total) + "\n")
        resi.write("    -----------------------------------\n")
        list_total[i]
        urutan += 1
        i += 1
    ...
    time.sleep(0.7)
    resi.write("\n---------------------------------------\n")
    resi.write("-> Total Harga      : " + uang(sum(list_total)) + "\n")

    if(sum(list_total) >= 200000):
        resi.write("-> Diskon           : 5%\n")
        resi.write("-> Potongan Diskon  : " + uang(int(nilai_diskon)) + "\n")
    else:
        resi.write("-> Diskon           : 0%\n")
        resi.write("-> Potongan Diskon  : " + uang(int(nilai_diskon)) + "\n")

    resi.write("-> Pajak\PPN        : 1%\n")
    resi.write("-> Tambahan Pajak   : " + uang(int(nilai_pajak)) + "\n")
    resi.write("---------------------------------------\n")
    resi.write("-> Total Biaya      : " + uang(int(total_akhir)) + "\n")
    resi.write("\n+============ TERIMA KASIH ===========+\n")
...
while True:   
    ...
    # validasi inputan tidak boleh <= 0 atau > 5
